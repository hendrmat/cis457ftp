  ...
  int sockfd, portno, n;
	...
	//send file to server
		...
		else
		{
			...
			read(sockfd,cBuff,255); //read ack from server(fixes consecutive write short message merge)
			//printf("RCV 1\n");
			write(sockfd, args[1], sizeof(args[1]));//send file name to server

			//check file name received/opened
			bzero(cBuff,256); //clear communication buffer
			read(sockfd,cBuff,255); //read ack for receival/opening of file on server side
			//printf("RCV 2\n");
	    		//check message confirming correct opening
            		if(strcmp(cBuff,"Name received")==0)
            		{
                		...
                		bzero(cBuff,256); //clear communication buffer
                		read(sockfd,cBuff,255);//get ack for size (fixes consecutive write short message merge)
                		//printf("RCV 3\n");
